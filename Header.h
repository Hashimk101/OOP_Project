...
public:
   ...
   virtual  int GetFrameWidth(SpriteSheet& S) = 0;
    ...
    void AnimateSprite(bool isMoving)
    {
        if (isMoving) 
        {
            if (animationClock.getElapsedTime().asMilliseconds() > 80) {
                currentFrame = (currentFrame + 1) % (GetFrameNum(SpriteTex[currentIndex].T));
...
public:
    Knuckles() : MySprite() 
    {
        ...
        if (!SpriteTex[1].T.loadFromFile("Data/Knuckles_Jog_Walk _L.png")) { //493x40 => 10 frames
         ...
         // IDLE
        if (!SpriteTex[2].T.loadFromFile("Data/Knuckles_Idle_ R.png")) {//40x40 => 1 frame
    ...
    bool movement(char** lvl) {
        ...
        if (Keyboard::isKeyPressed(Keyboard::Left))
        {
            ...
            if (leftCollision)
            {
                ...
                velocityX = std::max(velocityX - acceleration, -max_speed);
                currentIndex = 1;                  // Jog left strip
                ESprite.setTexture(SpriteTex[1].T);
                left = true;
                isMoving = true;
                AnimateSprite(isMoving);
            ...
            velocityX = std::min(velocityX + acceleration, max_speed);
            currentIndex = 3;                  // Jog right strip
            ESprite.setTexture(SpriteTex[3].T);
            left = false;
            isMoving = true;
            AnimateSprite(isMoving);
            ...
            // if on an edge  use edge hang animation
            if (isOnEdge(lvl)) {
                currentIndex = left ? 11 : 11; // hanging frame (you can swap indices if needed)
            ...
            // idle
            currentIndex = left ? 0 : 2;   // Idle left or idle right
            ESprite.setTexture(SpriteTex[currentIndex].T);
            AnimateSprite(false);
        }

        // JUMP / ROLL (Knuckles jump = roll) 
        if (onGround && Keyboard::isKeyPressed(Keyboard::Up)) {
            velocityY = -20;
            currentIndex = left ? 4 : 5;  // roll left or roll right
            ESprite.setTexture(SpriteTex[currentIndex].T);
            isMoving = true;
        }
        //  GLIDE in mid air 
        else if (!onGround && Keyboard::isKeyPressed(Keyboard::Up)) {
            currentIndex = 10;  // glide
            ESprite.setTexture(SpriteTex[10].T);
            isMoving = true;
        }

        // PUNCH 
        if (onGround && Keyboard::isKeyPressed(Keyboard::Down)) {
            if (Keyboard::isKeyPressed(Keyboard::Left)) {
                currentIndex = 6;  // punch left
                ESprite.setTexture(SpriteTex[6].T);
                isMoving = true;
                velocityX = 0;
            }
            else if (Keyboard::isKeyPressed(Keyboard::Right)) {
                currentIndex = 7;  // punch right
        ...
        // CLIMB (when touching wall) 
        if (isOnEdge(lvl) && Keyboard::isKeyPressed(Keyboard::Up)) {
            // override vertical movement
            velocityY = -1;  // slow climb speed
            currentIndex = 8;  // climb up
            ESprite.setTexture(SpriteTex[8].T);
            isMoving = true;
        }
